...
@pytest.mark.check_round_trip_flight
def test_check_round_trip_flight():
    """ Test case is:
        ...
        7. Click filter and set slider value
        8. Get departure times and check time in range
    ...
    logger.info("7. Click filter and set slider value")
    search_result_page = FlightSearchResultPage(driver)
    search_result_page.click_filter(2)
    search_result_page.set_slider_value(search_result_page.DEPARTURE_TIME_SLIDER_1, 600)
    search_result_page.set_slider_value(search_result_page.DEPARTURE_TIME_SLIDER_2, 1080)

    logger.info("8. Get departure times and check time in range")
    time_str_list = search_result_page.get_departure_times()
    for time_str in time_str_list:
        search_result_page.check_time_in_range(time_str, "10:00", "18:00")
